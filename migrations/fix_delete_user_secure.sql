...
BEGIN
    ...
    SELECT ARRAY_AGG(id) INTO project_ids FROM projects WHERE owner_id = target_user_id;
    ...
    -- FIX: Remove user from manager_ids arrays (Casting to TEXT because column is TEXT[])
    UPDATE projects 
    SET manager_ids = array_remove(manager_ids, target_user_id::text) 
    WHERE target_user_id::text = ANY(manager_ids);
    
    -- Also remove from departments manager_ids if it exists
    UPDATE departments
    SET manager_ids = array_remove(manager_ids, target_user_id::text)
    WHERE target_user_id::text = ANY(manager_ids);
    ...
    DELETE FROM daily_work_logs WHERE user_id = target_user_id;
